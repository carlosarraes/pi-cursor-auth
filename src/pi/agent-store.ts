...
export const CURSOR_STATE_ENTRY_TYPE = "pi-cursor-auth:state";
...
export const ensureAgentStore = async (
	...
	const entry = await ensureStore(CACHE_DIR, sessionId);
	return entry.store;
};
...
export const persistAgentStore = async (
	...
	const entry = await persistStore(CACHE_DIR, sessionId);
	if (!entry) {
		return null;
	}
...
export const restoreAgentStoreFromBranch = async (
	...
	const storeEntry = await ensureStore(CACHE_DIR, sessionId);
